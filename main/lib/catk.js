...
let version = require('../package.json')['_catkVersion'];
/*
    silly: 'rainbow',  
    input: 'grey',  
    verbose: 'cyan',  
			...
			if(stat.isDirectory(route_path) == true){//如果是目录
				...
				if(path.extname(_path) == '.js'){//如果是js文件
					...
					for(let fun in req){
						...
						route_list.push(`ALL: http://${config.local.host}:${config.local.port}`+'/'+_ps[1].split('Controller.js')[0].toLowerCase()+'/'+fun);
					}
				}
			}
...
function router_init(app_name){
	...
	if(config.local.auto_router) router_auto('./'+app_name+'/controllers/',app_name);
	...
	for (let route in routes){
		...
		if(typeof routes[route] == 'function'){
			...
			for(let i in router['stack']){
				let index = router['stack'][i]['methods'].indexOf(method.toUpperCase());
				if(result[1] == router['stack'][i]['path'] && index > -1){
					router['stack'][i]['methods'].splice(index,1);
				}
			}
			router[method](result[1],routes[route]);//创建路由
			route_list.push(`${method.toUpperCase()}: http://${config.local.host}:${config.local.port}${result[1]}`);
	...
	for(let i in router['stack']){//路由请求模式去重
		router['stack'][i]['methods'] = arr_unique(router['stack'][i]['methods']);
	}
}
...
`+'info'.cyan+`: Catk version : ${version}
`+'info'.cyan+`: Project name : ${pkg['name']}
`+'info'.cyan+`: Server runing in \`${process.cwd()}\`
`+'info'.cyan+`: To see your app, visit http://${config.local.host}:${config.local.port}
`+'info'.cyan+`: Port        : ${config.local.port}
`+'info'.cyan+`: -------------------------------------------------------`);
